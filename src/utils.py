...
HUMAN_READABLE_ABBREVIATION_SYMBOLS = ['K', 'M', 'G']

def approx_number(number):
    """
    Approximate a number to the nearest tenth.
    Args:
        number (float): The number to be approximated.
    Returns:
...
def human_readable_file_size(file_size):
    """
    ...
    if file_size // 1024 == 0:
        ...
        while  file_size >= 1024 and abbreviation_index < len(HUMAN_READABLE_ABBREVIATION_SYMBOLS) - 1:
            file_size /= 1024
            abbreviation_index += 1
        abbreviation_symbol = HUMAN_READABLE_ABBREVIATION_SYMBOLS[abbreviation_index]
    return str(approx_number(file_size)) + abbreviation_symbol
    
def human_readable_datetime(datetime_timestamp):
